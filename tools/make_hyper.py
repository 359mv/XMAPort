...
def detect_vndk_version(parent_dir):
    """Read ro.odm.build.version.sdk from base ROM (target) odm/etc *.prop files."""
...
def _set_sync_apex_changed(parent_dir, changed=True):
    status_path = os.path.join(parent_dir, "workspace", "sync_apex_changed.txt")
    try:
        os.makedirs(os.path.dirname(status_path), exist_ok=True)
        with open(status_path, "w", encoding="ascii") as f:
...
def sync_apex(parent_dir):
    ...
    vndk_version = detect_vndk_version(parent_dir)
    if vndk_version:
        g_vndk_version = vndk_version
    ...
    if source_apex:
        ...
        if copy_file_overwrite(target_apex, dest_path):
            changed = True
            _set_sync_apex_changed(parent_dir, True)
    ...
    changed = True
    _set_sync_apex_changed(parent_dir, changed)
...
def clean_vk_props(parent_dir):
    ...
    if not g_vndk_version:
        g_vndk_version = detect_vndk_version(parent_dir)
    if g_vndk_version == "36":
        LOG_INFO("Target VNDK version is 36, skipping vk props removal.")
        LOG_INFO("========================================")
        return 0
...
def sync_fps_list(parent_dir):
    LOG_INFO("")
    LOG_INFO("========================================")
    LOG_INFO("  Synchronize fpsList + smart_fps_value + support_max_fps")
    LOG_INFO("========================================")
        ...
        with open(target_xml, "r", encoding="utf-8", errors="ignore") as f:
            ...
            for line in f:
                ...
                if not in_array:
                    ...
                    if sf:
                        target_smart_fps = sf
                    mf = extract_int_attr_value(trimmed, "support_max_fps")
                    if mf:
                        target_max_fps = mf
                    continue
    ...
    if not target_items and not target_max_fps:
        LOG_INFO("No fpsList items or support_max_fps found in target XML, no modification made, skipping.")
        return 0
    ...
    LOG_INFO("Target support_max_fps: " + (target_max_fps if target_max_fps else "(not found)"))
    ...
    max_fps_need_add = (target_max_fps and not source_max_fps)
    if not source_items and not max_fps_need_add:
        LOG_INFO("No fpsList found in source XML and no support_max_fps to add, no modification made, skipping.")
        return 0
    ...
    max_fps_differ = (target_max_fps and source_max_fps and source_max_fps != target_max_fps)
    if not fps_differ and not smart_differ and not max_fps_need_add and not max_fps_differ:
        LOG_INFO("Source fpsList, smart_fps_value and support_max_fps already match target, no modification made, skipping.")
        return 0
    ...
    LOG_INFO("Source support_max_fps: " + (source_max_fps if source_max_fps else "(not found)"))
    ...
    for raw in lines:
        ...
        if not in_array:
            if target_smart_fps:
                ...
                if new_line != line:
                    line = new_line
                    replaced_smart += 1
            if target_max_fps and source_max_fps:
                new_line = replace_int_attr_value(line, "support_max_fps", target_max_fps)
                if new_line != line:
                    line = new_line
                    replaced_max_fps += 1
            kept_lines.append(line)
            ...
            if '<integer-array name="fpsList">' in trimmed:
                if target_items:
                    in_array = True
                    element_indent = line[:line.find('<integer-array')]
            continue
        trimmed = trim_str(line)
        if not indent_set:
            pos = line.find("<item>")
        ...
        if "</integer-array>" in trimmed:
            ...
            in_array = False
            kept_lines.append(line)
            if max_fps_need_add and not inserted_max_fps:
                ins_indent = element_indent if element_indent else indent
                kept_lines.append(ins_indent + '<integer name="support_max_fps">' + target_max_fps + '</integer>')
                inserted_max_fps = True
            continue
        if "<item>" in trimmed and "</item>" in trimmed:
            continue
        kept_lines.append(line)
    ...
    if max_fps_need_add and not inserted_max_fps:
        ...
        for line in kept_lines:
            if "<integer " in line:
                det_indent = line[:line.find("<integer")]
                break
        final_lines = []
        for line in kept_lines:
            if "</features>" in trim_str(line) and not inserted_max_fps:
                final_lines.append(det_indent + '<integer name="support_max_fps">' + target_max_fps + '</integer>')
                inserted_max_fps = True
            final_lines.append(line)
        kept_lines = final_lines
    ...
    LOG_INFO("fpsList synchronized in " + source_xml + " (fpsList items: " + int_to_str(len(target_items)) + ", smart_fps_value replaced: " + int_to_str(replaced_smart) + ", support_max_fps replaced: " + int_to_str(replaced_max_fps) + ", support_max_fps inserted: " + ("yes" if inserted_max_fps else "no") + "). Backup: " + backup_path)
    LOG_INFO("========================================")
    return 0
...
def run_speed_pipeline(parent_dir):
    _set_sync_apex_changed(parent_dir, False)
    steps = [
